    ...
    return (
      <div className="flex flex-initial flex-col recording pl-2 mb-10">
        <div className="text-inverse flex flex-initial items-center pl-6">
          ...
          {this.props.filename && (
            <div className="ml-10">{this.props.filename}</div>
          )}
          {!this.props.active && (
            <button
        ...
        {this.state.showDeleteConfirmationModal && (
          <RecordingDeleteConfirmationModal
            ...
            onConfirm={() =>
              this.props.dispatch(deleteRecording(this.props.id))
            }
          />
        )}
            ...
            <div className="flex flex-initial flex-col rounded bg-white text-black shadow m-2 p-4">
              ...
              <div className="mt-4 flex flex-wrap">
                {this.props.countingAreas &&
                  this.props.countingAreas
                    .entrySeq()
                    .map(([countingAreaId, countingAreaData], index) => (
                      <div
                        key={countingAreaId}
                      ...
                      >
                        <div className="flex items-center">
                          <h4 className="font-medium">
                            {countingAreaData.get("name")}
                          </h4>
                          <div
                            className="w-4 h-4 ml-2 rounded-full"
                            style={{
                              backgroundColor: getCounterColor(
                                countingAreaData.get("color")
                              ),
                            }}
                          ></div>
                          {countingAreaData.get("type") ===
                        ...
                        <div className="flex flex-initial flex-wrap mt-5 w-64">
                          {this.DISPLAY_CLASSES.slice(
                            0,
                            Math.min(this.DISPLAY_CLASSES.length, 6)
                          ).map((counterClass) => (
                            <div
