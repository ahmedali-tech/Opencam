...
class UIControls extends Component {
  ...
  render() {
    if (this.props.recordingStatus.isRecording) {
      var diff = Math.abs(
        new Date(this.props.recordingStatus.dateStarted) - new Date()
      );
      ...
      <React.Fragment>
        <div className="nav">
          ...
          <div className="recording-status">
            ...
            <div className="fps">
              {this.props.recordingStatus.currentFPS} FPS
            </div>
          </div>
          <div className="flex">
            <div className="nav-left mt-2 ml-2 shadow flex">
              <button
                className={`btn btn-default rounded-l ${
                  this.props.mode === MODE.LIVEVIEW ? "btn-default--active" : ""
                } ${
              ...
              {this.props.uiSettings.get("counterEnabled") &&
                (!this.props.recordingStatus.isRecording ||
                  ...
                  <button
                    className={`btn btn-default border-r border-l border-default-soft border-solid ${
                    ...
                    onClick={() =>
                      this.props.dispatch(setMode(MODE.COUNTERVIEW))
                    }
                  >
                    Zaehlung
                  </button>
            ...
            <div className="nav-right mt-2 mr-2 flex">
              <button
                className={`btn btn-default shadow rounded-l ${
                  this.props.mode === MODE.DATAVIEW ? "btn-default--active" : ""
                }`}
              ...
              <button
                className={`btn btn-default shadow rounded-r border-l border-default-soft border-solid ${
...
export default connect((state) => {
  return {
    ...
    isAtLeastOneCountingAreasDefined:
      state.counter.get("countingAreas").size > 0,
  };
})(UIControls);
